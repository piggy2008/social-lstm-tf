...
class SocialModel():
    ...
    def __init__(self, args, infer=False):
        ...
        # Output dimension of the model for many to seq
        self.output_size2 = 12 * 2
        ...
        for seq, frame in enumerate(frame_data):
            ...
            for ped in range(args.maxNumPeds):
                ...
                with tf.name_scope("increment_cost"):
                    ...
                    self.counter = tf.where(tf.not_equal(pedID, nonexistent_ped), tf.add(self.counter, self.increment), self.counter)
    ...
    def getSocialTensor(self, grid_frame_data, output_states):
        ...
        hidden_states = tf.concat(output_states, axis=0)
